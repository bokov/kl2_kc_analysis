...
default_font <- 'Times'; #'Times New Roman';
...
.par_eventplot$family = c('Times','Times New Roman');
...
#' 
# conclusions ---------------------------------------------------------
#' # Conclusion and Future Directions {#sec:nextsteps}
#' 
...
#' `r md$pbreak`
#' # Results {#sec:supp label="Appendix 3"}
#' 
#  event indicators -------------------------------------------------------------
...
#' 
#  diagnosis ====================================================================
mainvars <- union(c(v(c_preexist),'e_kc_i10','e_kc_i9',v(c_nephx)),v(c_death));
#kcpatients.first <- unique(subset(dat1,n_mult =='"01"')$patient_num);
kcpatients.notfirst <- unique(subset(dat2a
                                     ,grepl('[089][2-9]',n_mult))$patient_num);
kcpatients.met <- unique(subset(dat1,substr(n_a7sg,1,2)=='IV' | 
                                  substr(n_a6sg,1,2)=='IV')$patient_num);
...
much? {#tbl:diag_lag}');
e_table_diag<-e_table(dat2tte,'n_ddiag',v(c_kcdiag,dat2tte)
                      #,setdiff(c(v('c_kcdiag'),'n_fc'),'n_ddiag')
                      ,breaks=c(-30,30));
pander(e_table_diag[,-9],caption=.tc,missing='&nbsp;',justify='left'
       ,fmt='%3s\\\n%s\\\n%s',row.names=fs(rownames(e_table_diag))); 
#' 
#'
#+ etablediagfirst, results='asis'
.tc <- paste0('
For patients with NAACCR records and first-time occurence of cancer, how often 
do ICD9 or ICD10 codes for kidney cancer in the EMR lead or trail '
,fs('n_ddiag'),' and by how much? {#tbl:diag_lag_first}');
e_table_diag_first<- subset(dat2tte,!patient_num %in% kcpatients.notfirst) %>% 
  e_table('n_ddiag',v(c_kcdiag,dat2tte) 
          #setdiff(c(v('c_kcdiag')),'n_fc'),'n_ddiag')
          ,breaks=c(-30,30));
pander(e_table_diag_first[,-9],caption=.tc,missing='&nbsp;',justify='left'
       ,fmt='%3s\\\n%s\\\n%s',row.names=fs(rownames(e_table_diag_first))); 
...
.tc <- paste0("As can be seen in the table below, the EMR variables "
," _sometimes_ precede ",fs('n_ddiag')," by many weeks but they 
_usually_ follow ",fs('n_ddiag')," by more weeks than do ",fs('n_dsdisc')
," and ",fs('n_dsurg'),". Those two NAACCR variables never occur before "
...
variables. {#tbl:etablesurg}");
e_table_surg<-e_table(dat2tte,'n_rx3170',v(c_nephx),breaks=c(-30,30)
                      ,LFEXCLUDE = c('n_rx1270','n_rx1260','n_dsurg'
                                     ,'e_surgonc'));
...
_usually_ follow ",fs('n_ddiag')," by more weeks than do ",fs('n_dsdisc')
              ...
              ,fs('n_ddiag')," and usually occur within 2-8 weeks after it. This is 
another way of summarizing how much the EMR variables lag behind NAACCR 
variables. {#tbl:etablesurgfirst}");
e_table_surg_first<-subset(dat2tte,!patient_num %in% kcpatients.notfirst) %>% 
  e_table('n_rx3170',v(c_nephx),breaks=c(-30,30)
          ,LFEXCLUDE = c('n_rx1270','n_rx1260','n_dsurg','e_surgonc'));
pander(e_table_surg_first,caption=.tc,missing='&nbsp;',justify='left'
       ,fmt='%3s\\\n%s\\\n%s',row.names=fs(rownames(e_table_surg_first))); 
...
#' outline possible remedies to that. For now, `t_priorcond` can still be used 
...
#' `t_priorcond` can only be used to disqualify patients for having erroneous
...
e_table_recur<- subset(dat2tte,patient_num %in% kcpatients_rectype$Recurred) %>%
  e_table('n_drecur',setdiff(v('c_recur'),'n_drecur'),breaks=c(-30,30));
...
e_table_death <- e_table(dat2tte,'n_vtstat',v(c_death)
...
# allvars ----
#' ## All variables
#+ etableall, results='asis'
.tc <- paste0('All variables relative to diagnosis date {#tbl:etableall}');
e_table_all <- e_table(dat2tte,'n_ddiag',c('n_dob',mainvars,'n_fc','n_lc')
                       ,breaks=c(-30,30));
pander(e_table_all,caption=.tc,missing='&nbsp;',justify='left'
       ,fmt='%3s\\\n%s\\\n%s',row.names=fs(rownames(e_table_all))); 
#' 
#+ etableall01, results='asis'
.tc <- paste0('All variables relative to diagnosis date, only first-time 
tumors {#tbl:etableall01}');
e_table_all_01 <- subset(dat2tte,!patient_num %in% kcpatients.notfirst) %>% 
  e_table('n_ddiag',c('n_dob',mainvars,'n_fc','n_lc'),breaks=c(-30,30));
pander(e_table_all_01,caption=.tc,missing='&nbsp;',justify='left'
       ,fmt='%3s\\\n%s\\\n%s',row.names=fs(rownames(e_table_all_01))); 
#' 
#+ etableall01srg, results='asis'
.tc <- paste0('All variables relative to surgery date, only first-time 
tumors {#tbl:etableall01srg}');
e_table_all_01srg <- subset(dat2tte,!patient_num %in% kcpatients.notfirst) %>% 
  e_table('n_rx3170',c('n_dob',mainvars,'n_ddiag','n_fc','n_lc')
          ,breaks=c(-30,30));
pander(e_table_all_01srg,caption=.tc,missing='&nbsp;',justify='left'
       ,fmt='%3s\\\n%s\\\n%s',row.names=fs(rownames(e_table_all_01srg))); 
#+ etableall01srgnomet, results='asis'
.tc <- paste0('All variables relative to surgery date, only first-time 
tumors pre stage IV {#tbl:etableall01srgnomet}');
e_table_all_01srgnomet <- subset(dat2tte,!patient_num %in% kcpatients.notfirst & 
                              ! patient_num %in% kcpatients.met) %>% 
  e_table('n_rx3170',c('n_dob',mainvars,'n_ddiag','n_fc','n_lc')
          ,breaks=c(-30,30),FIRSTLAST=F);
pander(e_table_all_01srgnomet,caption=.tc,missing='&nbsp;',justify='left'
       ,fmt='%3s\\\n%s\\\n%s',row.names=fs(rownames(e_table_all_01srgnomet))); 

#' 
#' ::::: {#fig:histories00 custom-style="Image Caption"}
#+ histories,results='asis',fig.dim=c(8,16),fig.align='center'
.dat2clust <- (dat2hist <- dat2tte[,c(setdiff(rownames(e_table_all)
                                              ,c('Last Event','First Event'))
                                      ,'n_ddiag')] - dat2tte$n_ddiag) %>%
  apply(1,rank,na.last='keep') %>% t %>% dist %>% hclust;
.dat2histrange <- range(dat2hist[,-1],na.rm=T);
dat2hist$patient_num <- dat2tte$patient_num;
dat2hist$clust <- .dat2clust$order;
dat2hist  <- mutate(dat2hist
                    ,first=!patient_num%in%kcpatients.notfirst
                    ,met=patient_num%in%kcpatients.met
                    ,dsf=patient_num%in%kcpatients_rectype$`Disease-free`) %>%
  arrange(desc(first),met,desc(dsf),clust);
plot(0,0,pch='.'#,xlim=.dat2histrange
     ,xlim=c(-1000,3000),ylim=c(1,nrow(dat2hist)+1),col='#00000020'
     ,xlab='Days',ylab='Patient');
abline(h=(1:486)[!dat2hist$first & !dat2hist$dsf],col='#ff8800',lwd=0.1);
abline(h=(1:486)[dat2hist$first & !dat2hist$met & !dat2hist$dsf],col='#880000',lwd=0.1);
abline(h=(1:486)[dat2hist$first & dat2hist$met & !dat2hist$dsf],col='#ffff00',lwd=0.1);
abline(h=(1:486)[dat2hist$first & !dat2hist$met & dat2hist$dsf],col='#ff0000',lwd=0.1);
for(ii in v(c_kcdiag)) points(dat2hist[,ii],1:486,cex=0.3,col='#0000ff20');
for(ii in v(c_treat)[-3]) points(dat2hist[,ii],1:486,cex=0.1,col='#00000030');
for(ii in v(c_nephx_actual)) points(dat2hist[,ii],1:486,cex=0.2,col='#ff000040');
for(ii in v(c_met)) points(dat2hist[,ii],1:486,cex=0.1,col='#00880060');
for(ii in v(c_death)) points(dat2hist[,ii],1:486,cex=0.1,col='#ff00ff60');
for(ii in v(c_death)) points(dat2hist[,ii],1:486,cex=0.1,pch='+',col='#ff00ff60');
for(ii in v(c_cancer)) points(dat2hist[,ii],1:486,cex=0.1,col='#00ff8860');
cat("

Histories of all patients with registry records");
#' :::::
#' 
# A4 variables -----------------------------------------------------------------
#'

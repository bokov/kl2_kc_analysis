...
v <- function(var,dat
  ...
  # having values found in the colnames of 'dat' are returned. The 'formals' clause
  # of the 'if' statement is to let the user set local defaults for this function
  if(!missing(dat)||!identical(as.character(formals(sys.function())$dat),'')){
    dictionary <- dictionary[dictionary[[matchcol]]%in%colnames(dat),];}
